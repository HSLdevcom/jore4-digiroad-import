...
By executing `export_pgdump_routing.sh`, a separate routing schema is created
into the database. The data is read from the Digiroad schema and transformed
into the table structure defined in and used by the map-matching backend.
A database dump file named `digiroad_r_routing_<date>.pgdump` will be created as
an output into subdirectory named `workdir/pgdump`.

In addition to the dump file, a couple of toc list files (table of contents) are
also generated. A toc file may be passed as an argument to `pg_restore` command.
The toc files can be used to selectively apply what is being restored from the
dump, e.g. the entire schema with data or table data for selected tables only.
