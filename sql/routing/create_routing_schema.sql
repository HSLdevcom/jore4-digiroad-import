...
UPDATE :schema.infrastructure_link SET
    cost = CASE WHEN traffic_flow_direction IN ('bidirectional', 'forward') THEN ST_Length(geom_3d) ELSE -1 END,
    reverse_cost = CASE WHEN traffic_flow_direction IN ('bidirectional', 'backward') THEN ST_Length(geom_3d) ELSE -1 END;
