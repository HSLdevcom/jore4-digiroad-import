...
-- Exclude two links of which one is a duplicate and another is fixed for the
-- `ajosuunta` property.
UPDATE :schema.fix_layer_link_exclusion_geometry
SET geom = ST_GeomFromEWKT('SRID=3067;LINESTRING(385272 6672146 0 0, 385291 6672159 0 0)')
WHERE fid = 1;
...
mml_link_split_fractions AS (
    ...
    WHERE
        ...
        AND ST_StartPoint(link_supp.geom) <-> ST_StartPoint(link_mml.geom) > 0.5
        AND ST_StartPoint(link_supp.geom) <-> ST_EndPoint(link_mml.geom) > 0.5
        AND ST_EndPoint(link_supp.geom) <-> ST_StartPoint(link_mml.geom) > 0.5
        AND ST_EndPoint(link_supp.geom) <-> ST_EndPoint(link_mml.geom) > 0.5
...
mml_link_split_substrings AS (
    ...
    CROSS JOIN LATERAL (
        SELECT ST_Force3D(ST_LineSubstring(l.geom, r.start_fraction, r.end_fraction)) AS geom_3d
    ) substr
),
...
WITH drp_fix AS (
    SELECT
        row_number() OVER (
            ORDER BY matk_tunn, valtak_id
        ) AS rn,
        drp.*,
        fll.link_id AS new_link_id,
        CASE
            WHEN start_location_m_on_original_link IS NULL THEN drp.sijainti_m
