...
docker run --rm --link "${DOCKER_CONTAINER}":postgres -v ${CWD}/sql:/tmp/sql -v ${CWD}/workdir/pgdump:/tmp/pgdump \
  ${DOCKER_IMAGE} sh -c "$PSQL -v ON_ERROR_STOP=1 -f /tmp/sql/select_infra_links.sql -o /tmp/pgdump/${PGDUMP_OUTPUT} --tuples-only --no-align -v schema=${DB_IMPORT_SCHEMA_NAME}"
